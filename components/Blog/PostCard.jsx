...
const PostCard = ({ blog }) => {
  ...
  return (
    <motion.div variants={fadeInUp} className="bg-zinc-700 rounded-lg p-5">
      <div className="flex flex-col h-full">
        <div>
          ...
          <Link href={`/blog/${slug}`}>
            <a className="title text-lg sm:text-xl md:text-2xl font-bold text-zinc-200 hover:text-accent mt-3 block">
              {title}
            </a>
          </Link>
          <p className="meta-description text-zinc-400 tracking-tighter text-sm sm:text-base mb-5">
            {meta}
          </p>
        </div>
